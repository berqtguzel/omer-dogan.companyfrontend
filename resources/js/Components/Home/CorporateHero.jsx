...
export default function CorporateHero({ hero, areas, contactLink }) {
    ...
    const poster = buildResponsiveImage(hero.image, {
    ...
    return (
        <section
            className="group-hero group-hero--cinematic"
            aria-labelledby="group-hero-title"
        >
            {poster.src && (
                <Head>
                    <link
                        head-key="corporate-hero-preload"
                        rel="preload"
                        as="image"
                        href={poster.src}
            ...
            <div className="group-hero__backdrop" aria-hidden="true">
                ...
                {videoFailed && hero.image && (
                    <img src={poster.src || hero.image} alt="" />
                )}
            ...
            <Container>
                <div className="group-hero__content">
                    <p className="group-hero__kicker">
                        <span />
                        {t("corporateHome.areasEyebrow")}
                    </p>
                    <h1 id="group-hero-title" className="corporate-heading">
                        {title}
                    </h1>
                    {hero.description && (
                        <p className="group-hero__description">
                            {hero.description}
                        </p>
                    )}
                    <div className="group-actions">
                        <SiteLink
                            item={hero.primary}
                            className="corporate-button corporate-button--light"
                            data-track-key="home_group_discover"
                            data-track-location="hero"
                            data-track-action="navigate"
                        >
                            {hero.primaryLabel || t("corporateHome.discover")}
                            <span aria-hidden="true">↗</span>
                        </SiteLink>
                        <SiteLink
                            item={contactLink}
                            className="group-text-link group-text-link--light"
                        >
                            {t("corporateHome.contact")}
                            <span aria-hidden="true">↗</span>
                        </SiteLink>
                    </div>
                </div>
                {areas.length > 0 && (
                    <nav
                        className="group-hero__directory"
                        aria-label={t("corporateHome.areasEyebrow")}
                    >
                        <ol>
                                ...
                                <li key={area.id}>
                                    <a href={`#area-${area.id}`}>
                                        ...
                                        <span>
                                            {area.title ||
                                                t(
                                                    `corporateHome.areas.${area.id}.title`,
                                                )}
                                        </span>
                                        <span aria-hidden="true">↗</span>
                                    </a>
