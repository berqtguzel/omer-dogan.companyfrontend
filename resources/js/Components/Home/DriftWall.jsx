...
const DriftWall = ({
  ...
  useEffect(() => {
    ...
    if (!running || reduced || speed === 0) return;
    const animate = ts => {
  ...
  }, [baseVelocities, columnMeta, pauseOnHover, parallax, reduced, running, speed, applyPlaneTransform]);
  ...
  const renderTile = (item, id, colIndex) => {
    ...
    if (item.link?.href || item.href) {
      return (
        <SiteLink key={id} item={item.link || { href: item.href }} {...commonProps}>
          {inner}
        </SiteLink>
      );
    }
